...
class model():
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            sampled_file_name=sample_files_test()
            print(sampled_file_name)
            for i in range(2):
                content_imgs = sample(64, 1, self.args.content_dir, [sampled_file_name[i]])
                fake_ = sess.run(self.fake_, feed_dict={self.input_img:content_imgs})
                fake_=np.array(fake_)
                print(sampled_file_name[i]+': OK')
                visualize_test(fake_,sampled_file_name[i])
...
if __name__ == '__main__':
    ...
    argp.add_argument("--content_dir", dest="content_dir", type=str, default="./sun/")
    argp.add_argument("--style_dir", dest="style_dir", type=str, default="./target/")
    argp.add_argument("--content_font_dir", dest="content_font_dir", type=str, default="./togoshi-mono-20080629/togoshi-mono.ttf")
    argp.add_argument("--style_font_dir", dest="style_font_dir", type=str, default="./nicokaku-plus/nicokaku_v1.ttf")
