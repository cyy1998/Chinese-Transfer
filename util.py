...
def sample(size, channel, fontimg_path, file_names):
    ...
    for file_name in file_names:
        img = np.expand_dims(encode(scipy.misc.imread(fontimg_path+file_name+'.png', mode='L').astype(np.float32)), -1)
...
def visualize_test(fakeY, name):
    decode = lambda x: (x+1.)/2
    #decode = lambda x: x/1
    img = np.array(fakeY).reshape((64,64))
    scipy.misc.imsave('./saved/{}.bmp'.format(name), decode(img))

def mk_font_imgs(font_path, save_path, font_size = 42, text_lists = 'font.txt'):
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    
    with open(text_lists,'r') as fs:
        chars = fs.readlines()
        print(chars)
...
def sample_files_function(batch_size, text_lists = 'font.txt'):
    with open(text_lists,'r') as fs:
        chars = fs.readlines()
...
def sample_files_test(text_lists = 'font_test.txt'):
    with open(text_lists,'r') as fs:
        chars = fs.readlines()
